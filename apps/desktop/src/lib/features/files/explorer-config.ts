...
export async function ensureExplorerConfigPath(rootPathOverride?: string | null) {
...
export async function writeExplorerConfig(config: Partial<ExplorerConfig>, rootPathOverride?: string | null) {
  const configPath = await ensureExplorerConfigPath(rootPathOverride);
  if (!configPath) return defaultExplorerConfig;
...
export function clampExplorerSidebarWidth(width: number) {
...
async function readLegacyExplorerConfig(rootPath: string): Promise<ExplorerConfig> {
  const configPath = `${rootPath}/${brainstormFolderName}/${legacyConfigFileName}`;

  try {
    const configExists = await invoke<boolean>('path_exists', { path: configPath });
    if (!configExists) return defaultExplorerConfig;
