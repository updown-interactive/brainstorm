...
pub struct EncryptedDatabaseStore {
    pool: SqlitePool,
    key: [u8; 32],
}
...
impl EncryptedDatabaseStore {
    ...
    pub fn new(pool: SqlitePool) -> Self {
        let machine_identity =
            machine_uid::get().unwrap_or_else(|_| "brainstorm-local-machine".to_string());
...
impl CredentialStore for EncryptedDatabaseStore {
    ...
    async fn get(&self, credential_id: &str) -> Result<Option<String>, CredentialError> {
        let row = sqlx::query_as::<_, (Vec<u8>, Vec<u8>)>(
            "SELECT ciphertext, nonce FROM llm_credentials WHERE credential_id = ?",
        )
        .bind(credential_id)
    ...
    async fn delete(&self, credential_id: &str) -> Result<(), CredentialError> {
        sqlx::query("DELETE FROM llm_credentials WHERE credential_id = ?")
            .bind(credential_id)
            .execute(&self.pool)
            .await
            .map_err(CredentialError::Storage)?;
...
impl CredentialService {
    ...
    pub async fn save_api_key(
        ...
        self.store.set(credential_id, api_key.trim()).await
    }
    pub async fn get_api_key(&self, credential_id: &str) -> Result<String, CredentialError> {
        self.store
            .get(credential_id)
            .await?
