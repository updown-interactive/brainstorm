...
import type { GraphBounds, GraphPoint, LayoutNode, RenderEdge } from './graph-types';
...
export class GraphRuntimeStore {
  ...
  rebuildVisible(bounds: GraphBounds) {
  ...
  hitTest(point: GraphPoint, zoom: number) {
...
function buildDegreeMap(fileNodes: KnowledgeNode[], fileEdges: KnowledgeEdge[]) {
...
function resolveRenderEdges(sourceEdges: KnowledgeEdge[], nodeById: Map<string, LayoutNode>) {
...
function searchableText(node: KnowledgeNode, folder: string) {
...
function isNodeOutsideView(node: LayoutNode, bounds: GraphBounds, margin: number) {
...
function isEdgeOutsideView(source: LayoutNode, target: LayoutNode, bounds: GraphBounds, margin: number) {
