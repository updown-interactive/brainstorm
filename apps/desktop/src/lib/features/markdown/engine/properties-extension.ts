...
class PropertiesWidget extends WidgetType {
	...
	toDOM(view: EditorView) {
		...
		if (displayMode === 'hover') {
			wrap.classList.add('is-hover-mode');
		}

		const header = document.createElement('div');
		header.className = 'cm-properties-header';
		...
		body.className = 'cm-properties-body';
		if (isCollapsed) {
		...
		body.appendChild(this.createAddProperty(view));
	...
	private async openOrCreateLink(value: string, view: EditorView) {
		...
		editorStore.openFile(file.path, file.name, false);
		view.focus();
	}

	private async createLinkedFile(value: string) {
		const state = get(fileTreeState) as FileTreeState;
...
export function buildPropertiesDecorations(state: EditorState): DecorationSet {
