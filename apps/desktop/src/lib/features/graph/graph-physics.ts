...
import type { LayoutNode, RenderEdge } from './graph-types';
...
export class GraphPhysicsEngine {
  ...
  start(alpha = 0.55, boostFrames = 0) {
  ...
  private applyChargeForce(nodes: LayoutNode[], settings: PerceptualForceSettings) {
    if (nodes.length <= 220) {
  ...
  private repelPair(left: LayoutNode, right: LayoutNode, settings: PerceptualForceSettings) {
  ...
  private applyLinkForce(settings: PerceptualForceSettings) {
  ...
  private applyCenterForce(nodes: LayoutNode[], settings: PerceptualForceSettings) {
  ...
  private applyCollisionForce(nodes: LayoutNode[]) {
    if (nodes.length > 220) {
  ...
  private applyGridCollisionForce(nodes: LayoutNode[]) {
    const cellSize = 70;
    const grid = new Map<string, LayoutNode[]>();
...
export function centerNodes(nodes: LayoutNode[]) {
  if (nodes.length === 0) return;
...
function collidePair(left: LayoutNode, right: LayoutNode) {
...
function limitNodeVelocity(node: LayoutNode) {
...
function sanitizeNodePosition(node: LayoutNode) {
...
function seededJitter(left: string, right: string) {
  return ((stableHash(`${left}:${right}`) % 200) - 100) / 1000 || 0.01;
}

function seededCoordinate(id: string, axis: string) {
...
function graphCentroid(nodes: LayoutNode[]) {
...
function expCurve(value: number, curve: number) {
