...
import { todayString } from './frontmatter';
...
export async function listSharedTags() {
  const config = await readPropertyConfig();
  tagCache = normalizeSharedTags(config.tags ?? []);
  return tagCache;
}

export async function ensureSharedTags(names: string[]) {
  ...
  tagCache = nextTags;
  await writePropertyConfig({ ...config, tags: nextTags });
  return tagCache;
}
