<script lang="ts">
  ...
  export let onOpenKnowledgeSource: (relativePath: string) => void;
  ...
  const workingTypeLabel = (workingType: string): string => workingType === 'knowledge_grounded' ? 'Knowledge grounded' : 'General generation';
    ...
    {:else}
      ...
      {#each messages as message}
        <article id="chat-message-{message.id}" class:knowledge-source-target={highlightedMessageId === message.id} class="chat-message {message.role === 'assistant' ? 'assistant-message' : 'user-message'}">
          <div class="message-body"><div class="message-markdown">{#if message.role === 'assistant'}{#if message.status === 'streaming' && !message.content}<div class="working-live-status">Working…</div>{:else}{@html renderMarkdown(isKnowledgeResponse(message) ? knowledgeBody(message.content) : displayMessageContent(message.content))}{/if}{:else if message.content.startsWith('Create notes #')}<p><span>Create notes </span><button class="knowledge-source-mention" type="button" onclick={() => scrollToSourceResponse(message.id)} aria-label="View the assistant response used to create this note">{message.content.slice('Create notes '.length)}</button></p>{:else}<p>{escapeMessageText(message.content)}</p>{/if}</div>{#if message.role === 'assistant' && message.metadata?.working}<details class="working-trace" open={message.status === 'streaming'}><summary><span class="working-trace-title">{message.status === 'streaming' ? 'Working' : 'Work completed'}</span><span class="working-trace-type">{workingTypeLabel(message.metadata.working.workingType)}</span></summary><ol>{#each message.metadata.working.steps as step}<li class:working-step-active={step.status === 'working'} class:working-step-completed={step.status === 'completed'}><span class="working-step-marker">{step.status === 'completed' ? '✓' : step.status === 'working' ? '•' : '○'}</span><span>{step.label}</span></li>{/each}</ol></details>{/if}{#if message.role === 'assistant' && message.metadata?.knowledge}<details class="knowledge-used-indicator"><summary>✦ Used {message.metadata.knowledge.sources.length} knowledge source{message.metadata.knowledge.sources.length === 1 ? '' : 's'}</summary><ul>{#each message.metadata.knowledge.results as result}<li><button class="knowledge-source-link" type="button" onclick={() => onOpenKnowledgeSource(result.filePath)} title={`Open ${result.filePath}`}>{result.heading || result.title || result.filePath}</button><span class="knowledge-source-path">{result.filePath}</span></li>{/each}</ul></details>{/if}{#if message.role === 'assistant'}<span class="message-status">{message.status}</span>{/if}</div>
          <div class="message-actions">
            {#if message.role === 'assistant' && message.metadata?.knowledge?.results.length}{#each message.metadata.knowledge.results.slice(0, 1) as result}<button class="knowledge-open-source-button" type="button" onclick={() => onOpenKnowledgeSource(result.filePath)} title={`Open ${result.filePath}`}>Open {result.heading || result.title || 'source'}</button>{/each}{/if}{#if message.role === 'assistant' && isKnowledgeResponse(message) && message.id !== knowledgeMessage?.id}{#if message.id === knowledgePreview?.message.id}<button class="knowledge-preview-button knowledge-response-preview-button" type="button" onclick={openKnowledgePreview}>Preview knowledge</button>{:else}<button class="knowledge-preview-button knowledge-response-preview-button" type="button" onclick={() => void previewPersistedKnowledge(message)}>Preview knowledge</button>{/if}{:else if message.role === 'assistant' && message.id !== knowledgeMessage?.id}<button class="knowledge-action-button" type="button" aria-label="Create knowledge note from this assistant response" title={creatingKnowledgeMessageId === message.id ? 'Creating knowledge note' : 'Create note from this response'} disabled={creatingKnowledgeMessageId === message.id} onclick={() => void createKnowledgeResponse(message)}><FilePlus size={13} /></button>{/if}<button class="message-copy-button" type="button" aria-label="Copy message" title="Copy message" onclick={() => void copyMessage(message.id, message.content)}>{#if copiedMessageId === message.id}<Check size={12} />{:else}<Copy size={12} />{/if}</button>
          </div>
        </article>
      {/each}
