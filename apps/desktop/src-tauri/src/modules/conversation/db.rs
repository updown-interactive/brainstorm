...
pub async fn init(pool: &SqlitePool) -> Result<(), AppError> {
    sqlx::query("CREATE TABLE IF NOT EXISTS conversations (id TEXT PRIMARY KEY NOT NULL, project_id TEXT NOT NULL, title TEXT NOT NULL, agent_id TEXT, provider_config_id TEXT, model TEXT, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, last_message_at INTEGER, archived INTEGER NOT NULL DEFAULT 0, metadata TEXT, FOREIGN KEY(project_id) REFERENCES projects(id) ON DELETE CASCADE)").execute(pool).await?;
...
pub async fn conversation(
    ...
    sqlx::query_as("SELECT id, project_id, title, agent_id, provider_config_id, model, created_at, updated_at, last_message_at, archived FROM conversations WHERE project_id = ? AND id = ?").bind(project_id).bind(id).fetch_optional(pool).await?.ok_or_else(|| AppError::Internal("Conversation not found for project".into()))
}
