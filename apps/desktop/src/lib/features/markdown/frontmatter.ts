...
const LIST_PROPERTIES = new Set(['tags', 'aliases', 'links', 'contributors']);
const DATE_PROPERTIES = new Set(['created', 'updated', 'published', 'date']);
const NUMBER_PROPERTIES = new Set(['priority', 'estimated_hours']);
const BOOLEAN_PROPERTIES = new Set(['favorite', 'reviewed', 'draft']);
const MULTILINE_PROPERTIES = new Set(['summary', 'description']);
const ENUM_PROPERTIES = new Map<string, string[]>([
...
function replaceFrontmatter(doc: string, data: Record<string, unknown>) {
  ...
  return `${frontmatter}${doc.slice(range.bodyFrom)}`;
}
...
function validateProperty(property: FrontmatterProperty) {
...
function findDuplicateTopLevelKeys(yaml: string) {
